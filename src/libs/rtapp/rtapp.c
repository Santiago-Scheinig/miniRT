...
/*   Updated: 2026/03/22 17:39:54 by sscheini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	rtapp_init(int argc, char **argv, t_rtapp *app)
{
	...
	if (argc != 2)
		return (RTERR_FAILURE);//errlog("[ms][pid]: Invalid number of arguments\nOnly one .rt file can be proccessed at the same time\n");
	aux = parser(argv);//errlog("[ms][pid]: Invalid '%c' character on line '%i' in *.rt file\n") || errlog("[ms][pid]: Invalid parameters for '%s' objects on line '%i' in *.rt file\nPlease verify the correct format:\n['%s' format]: xxxxx";
	if (!aux)
		return (RTERR_FAILURE);
	//log("[ms][pid]: *.rt file validated.")
	if (init_log(app))
		return (RTERR_SUCCESS);//errlog("[ms][pid]: Unable to initialize log and errlog files\n");
