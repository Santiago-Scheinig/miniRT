...
void    info_cy(t_object *obj)
{
    ...
    data = (t_elem_cylinder *)obj->data;
    rtlog(RT_LOG, 0, "== CYLINDER ==");
    rtlog(RT_LOG, 0, "diam: %f | height: %f", data->diam, data->height);
    rtlog(RT_LOG, 0, "material rgb: %i", obj->material.rgb);
    rtlog(RT_LOG, 0, "inv:   [%f %f %f %f]", ...row 0...);
