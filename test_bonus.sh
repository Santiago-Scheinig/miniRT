...
NORM_DIRS=(
  ...
  ./libs/librt/src/rtelm/cone
  ./libs/librt/src/rtelm/paraboloid
  ./libs/librt/src/rtelm/hyperboloid
...
if [ "$RESOLUTION" == "DK" ]; then
  RES="make bonus W_WIDTH=1920 W_HEIGHT=1080"
elif [ "$RESOLUTION" == "2K" ]; then
  RES="make bonus W_WIDTH=2560 W_HEIGHT=1440"
elif [ "$RESOLUTION" == "4K" ]; then
  RES="make bonus W_WIDTH=3840 W_HEIGHT=2160"
elif [ "$RESOLUTION" == "V" ]; then
  RES="make bonus W_WIDTH=300 W_HEIGHT=300"
  VALGRIND=1
else
  echo "Usage: $0 [-norm] [DK, 2K, 4K, V]"
  exit 1
