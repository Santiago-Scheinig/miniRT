...
**Flow:** `sub-branch` → PR → `feat/` → PR → `dev` → PR → `main`

Each arrow is a Pull Request. Your partner reviews before anything merges.
...
- base: `feat/parser` ← compare: `parser/camera`
- Request your partner as reviewer
- Once approved → **Squash and merge** → delete the sub-branch

### 4. Sync every morning
